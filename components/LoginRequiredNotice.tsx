  ...
  return (
    <div className="rounded-lg border border-brand-100 bg-brand-50 p-6 text-center">
      ...
      <div className="mt-4 flex justify-center gap-2">
        <Link href={`/giris?next=${next}`} className="inline-flex min-h-11 items-center rounded-lg bg-brand-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-800">
          Giriş Yap
        </Link>
        <Link href={`/kayit?next=${next}`} className="inline-flex min-h-11 items-center rounded-lg bg-paper px-4 py-2 text-sm font-semibold text-brand-700 ring-1 ring-brand-200 transition hover:bg-brand-50">
          Kayıt Ol
        </Link>
      </div>
    </div>
