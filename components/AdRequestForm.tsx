...
export default function AdRequestForm() {
  ...
  return (
    <form onSubmit={handleSubmit} aria-busy={status === "loading"} aria-describedby={status === "error" ? "ad-request-error" : undefined} className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="ad-name" className={labelCls}>Ad Soyad <span aria-hidden="true" className="text-red-500">*</span></label>
          <input id="ad-name" name="name" required autoComplete="name" placeholder="Adınız ve soyadınız" className={inputCls} />
        </div>
        <div>
          <label htmlFor="ad-company" className={labelCls}>Firma</label>
          <input id="ad-company" name="company" autoComplete="organization" placeholder="Firma adı" className={inputCls} />
        </div>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="ad-phone" className={labelCls}>Telefon <span aria-hidden="true" className="text-red-500">*</span></label>
          <input id="ad-phone" name="phone" required type="tel" inputMode="tel" autoComplete="tel" placeholder="05__ ___ __ __" className={inputCls} />
        </div>
        <div>
          <label htmlFor="ad-email" className={labelCls}>E-posta</label>
          <input id="ad-email" name="email" type="email" autoComplete="email" placeholder="ornek@eposta.com" className={inputCls} />
        </div>
      </div>
      <div>
      ...
      {status === "error" && (
        <p id="ad-request-error" role="alert" className="rounded-[10px] bg-red-50 px-3.5 py-2.5 text-sm font-medium text-red-700 ring-1 ring-red-200">{error}</p>
      )}
      <button
        type="submit"
