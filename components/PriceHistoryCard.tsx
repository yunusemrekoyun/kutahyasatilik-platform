  ...
  return (
    <section className="rounded-control bg-paper p-6 ring-1 ring-stone">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h2 className="text-lg font-bold text-ink">Fiyat Geçmişi</h2>
          <p className="text-xs text-muted">Şeffaf fiyatlandırma — bu ilanın fiyat değişimleri</p>
        </div>
      </div>

      {diffPct !== 0 && (
        <div
          className={`mt-4 inline-flex items-center gap-2 rounded-control px-3 py-2 text-sm font-semibold ${
      ...
      <ul className="mt-4 space-y-2">
        {history
          ...
          .map((h, i, arr) => {
            ...
            return (
              <li key={i} className="flex items-center justify-between border-b border-stone py-2 text-sm last:border-0">
                <span className="text-muted">{formatDate(h.createdAt)}</span>
                <span className="flex items-center gap-2">
                  <span className="font-semibold text-ink">{formatPrice(h.price, currency)}</span>
                  {delta !== 0 && (
