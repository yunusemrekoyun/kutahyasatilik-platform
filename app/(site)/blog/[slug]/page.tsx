...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-10">
      ...
      {post.coverImage && (
        <div className="relative mt-6 aspect-[16/9] overflow-hidden rounded-xl bg-slate-100 ring-1 ring-slate-200">
      ...
      {related.length > 0 && (
        <section className="mt-12">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {related.map((r) => (
              <Link key={r.id} href={`/blog/${r.slug}`} className="group rounded-xl bg-white p-4 ring-1 ring-slate-200 hover:shadow-card hover:ring-brand-200">
