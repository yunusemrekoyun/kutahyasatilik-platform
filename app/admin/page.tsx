    ...
    <div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <div className={`${adminCard} p-6`}>
          <h2 className="font-semibold text-ink">En Çok Görüntülenen İlçeler</h2>
          <div className="mt-4 space-y-3">
            {analytics.districtViews.length === 0 && <p className="text-sm text-muted/70">Henüz veri yok.</p>}
        ...
        <div className={`${adminCard} p-6`}>
          <h2 className="font-semibold text-ink">En Çok Görüntülenen İlanlar</h2>
        ...
        <div className={`${adminCard} p-6`}>
          <h2 className="font-semibold text-ink">En Çok Talep Alan İlanlar</h2>
          <div className="mt-4 space-y-2">
            {leadsByListing.length === 0 && <p className="text-sm text-muted/70">Henüz talep yok.</p>}
        ...
        {/* Dönüşüm kaynakları */}
        <div className={`${adminCard} p-6`}>
          <h2 className="font-semibold text-ink">Dönüşüm Kaynakları (Son 30 Gün)</h2>
          <p className="text-xs text-muted/70">Google Ads dönüşüm takibi (utm_source / gclid)</p>
          <div className="mt-4 space-y-2">
      ...
      <div className={`mt-6 ${adminCard} p-6`}>
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-ink">Son Gelen Talepler</h2>
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone text-left text-xs text-muted">
            ...
            <tbody>
              {recentLeads.length === 0 && (
                <tr><td colSpan={4} className="py-4 text-center text-muted/70">Henüz talep yok.</td></tr>
              )}
              {recentLeads.map((l) => (
                <tr key={l.id} className="border-b border-stone/50">
                  <td className="py-2.5 pr-4 text-muted whitespace-nowrap">{formatDateTime(l.createdAt)}</td>
